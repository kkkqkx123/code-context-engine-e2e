...
use cce_e2e_tests::judgments::jackson_core::jackson_core_relevance_judgments;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let all_judgments = jackson_core_relevance_judgments();
    ...
    let paths = BenchmarkPaths::new("jackson_core");
    ...
    if all_results.is_empty() {
        eprintln!("No baseline data loaded. Run `gen_bench_jackson_core` first.");
        return Ok(());
    }
