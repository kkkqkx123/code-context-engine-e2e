//! Export NL documents for Python fixtures.
//!
//! Output:
...
async fn main() {
    ...
    let specs: Vec<(&str, cce_e2e_tests::FixtureSpec)> = vec![
        ("flask", cce_e2e_tests::FixtureSpec::python_flask()),
        ("basic", cce_e2e_tests::FixtureSpec::python_basic()),
        (
            "index_sidecar",
            cce_e2e_tests::FixtureSpec::python_index_sidecar(),
        ),
    ];

    let fixtures: Vec<(&str, TestFixture)> = specs
        .into_iter()
        .map(|(name, spec)| {
            let fixture = TestFixture::load(spec)
    ...
    for (name, fixture) in &fixtures {
        ...
        let mut emb_by_file: BTreeMap<String, Vec<_>> = BTreeMap::new();
        let mut bm25_by_file: BTreeMap<String, Vec<_>> = BTreeMap::new();
        ...
        for entry in &file_entries {
                ...
                .await
            {
                ...
                Err(e) => eprintln!("  BM25 process error: {}", e),
            }
        }

        // === Summary output (markdown NL docs) ===
        ...
        summary.push_str(&format!("# Export Output: {}\n\n", name));
        ...
        println!("  Summary: {} files", copied_files.len());

        // === Chunks output (segmentation verification) ===
        if !emb_by_file.is_empty() || !bm25_by_file.is_empty() {
            ...
            let chunking_emb_mgr = OutputManager::builder()
                .category(OutputCategory::Scenarios)
                .language("python")
                .scenario(format!("chunks/{}/emb", name))
                .build();
            let chunking_emb_dir = chunking_emb_mgr
                .ensure_output_dir()
                .expect("Failed to create chunking emb output dir");

            for (file_path, chunks) in &emb_by_file {
                ...
                let file_dir = path
                    ...
                    .unwrap_or_default();
                let target_dir = chunking_emb_dir.join(&file_dir);
                std::fs::create_dir_all(&target_dir).ok();
                let target_path = target_dir.join(&file_name);
                std::fs::write(&target_path, &content).ok();
            }

            let chunking_bm25_mgr = OutputManager::builder()
                .category(OutputCategory::Scenarios)
                .language("python")
                .scenario(format!("chunks/{}/bm25", name))
                .build();
            let chunking_bm25_dir = chunking_bm25_mgr
                .ensure_output_dir()
                .expect("Failed to create chunking bm25 output dir");

            for (file_path, chunks) in &bm25_by_file {
                let path = Path::new(file_path);
                let content = render_chunking_file(chunks, file_path);
                let file_name = path.file_name().unwrap().to_string_lossy();
                let file_name = format!("{file_name}.txt");
                let file_dir = path
                    ...
                    .map(|p| p.to_string_lossy().to_string())
                    .unwrap_or_default();
                let target_dir = chunking_bm25_dir.join(&file_dir);
                std::fs::create_dir_all(&target_dir).ok();
                let target_path = target_dir.join(&file_name);
                std::fs::write(&target_path, &content).ok();
            }

            let total_emb: usize = emb_by_file.values().map(Vec::len).sum();
            let total_bm25: usize = bm25_by_file.values().map(Vec::len).sum();
            println!("  Chunks: emb={} bm25={}", total_emb, total_bm25);
        }
    }

    println!("\n=== All Python fixtures exported ===");
}
