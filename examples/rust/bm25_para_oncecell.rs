...
use cce_e2e_tests::bm25_parameter_benchmark::{
    aggregate_by_parameter_set, run_once_cell_bm25_parameter_sweep,
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let output_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("outputs/benchmark/once_cell/bm25_parameter_sweep");
    let observations = run_once_cell_bm25_parameter_sweep(&output_dir).await?;
