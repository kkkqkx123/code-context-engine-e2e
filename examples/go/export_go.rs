//! Export NL documents for Go fixtures (basic, interfaces, control_flow, gin).
//!
//! Output:
...
async fn main() {
    ...
    let specs: Vec<(&str, FixtureSpec)> = vec![
        ("basic", FixtureSpec::go_basic()),
        ("interfaces", FixtureSpec::go_type_inference_interfaces()),
        (
            "control_flow",
            FixtureSpec::go_type_inference_control_flow(),
        ),
        ("gin", FixtureSpec::go_gin()),
    ];
